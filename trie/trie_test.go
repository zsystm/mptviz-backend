...
func testMissingNode(t *testing.T, memonly bool, scheme string) {
	...
	val, err := trie.Get([]byte("120000"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if !bytes.Equal(val, []byte("qwerqwerqwerqwerqwerqwerqwerqwer")) {
		t.Errorf("Unexpected value: %x", val)
	}
	trie, _ = New(TrieID(root), triedb)
	val, err = trie.Get([]byte("120099"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if val != nil {
	...
	trie, _ = New(TrieID(root), triedb)
	val, err = trie.Get([]byte("123456"))
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if !bytes.Equal(val, []byte("asdfasdfasdfasdfasdfasdfasdfasdf")) {
		t.Errorf("Unexpected value: %x", val)
	}
	trie, _ = New(TrieID(root), triedb)
